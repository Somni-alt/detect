...
export async function scoreCandidateForJob(candidateId: string, jobId: string, text: string) {
  ...
  const rationale =
    `${Math.round(mustHaveCoverage * 100)}% must-have; sim ${semanticSimilarity.toFixed(2)}; ${years} ans d'exp; ` +
    (missing.length ? `manque: ${missing.slice(0, 3).join(', ')}` : 'couverture complète');
