...
export async function scoreCandidateForJob(candidateId: string, jobId: string, text: string) {
  ...
  const mustHaveCoverage = mustHave.length
    ? mustHave.filter(s => skills.includes(s.toLowerCase())).length / mustHave.length
    : 0;

  const semanticSimilarity = skills.length
    ? Math.min(1, skills.length / (mustHave.length + 5))
    : 0.1;

  const locationMatch = true;

  const score =
    0.45 * mustHaveCoverage +
    0.30 * semanticSimilarity +
    0.15 * normalizeYears(years) +
    0.10 * (locationMatch ? 1 : 0);
  ...
  rationaleParts.push(`${Math.round(mustHaveCoverage * 100)}% must-have`);
  rationaleParts.push(`sim ${semanticSimilarity.toFixed(2)}`);
  rationaleParts.push(`${years} ans d'exp`);
  if (missing.length) rationaleParts.push(`manque: ${missing.slice(0, 3).join(', ')}`);
